...
static unsigned char bEUKOIsWarnLowActive(struct LocalWarnDat *ptLWD);

static unsigned char bUSIsWarnLowActive(struct LocalWarnDat *ptLWD);

static unsigned char bPRCIsWarnLowActive(struct LocalWarnDat *ptLWD);

unsigned char bEuF( struct LocalWarnDat *ptLWD, unsigned char ucWarnCfg)
{
...
static unsigned char bEUKOIsWarnLowActive(struct LocalWarnDat *ptLWD)
{
  ...
  {
   ucRet = ucSetWarnBitWN(ptLWD->tHFD.tHF.ucId, ucEuFIxc);
  }
  else{
   if(ucGetWarnBitWN(ptLWD->tHFD.tHF.ucId, ucEuFIxc) == 1)
   {
      if(ptLWD->tHFD.tHF.ucP >= ptLWD->tSD.ucPSoll)
      {
        ClearWarnBitWN(ptLWD->tHFD.tHF.ucId, ucEuFIxc);
        ucRet = 0;
      }
      else{
...
static unsigned char bPRCIsWarnLowActive(struct LocalWarnDat *ptLWD)
{
  ...
  if(u8ReceivedRelativePressure <= ucPCompareSETCond)
  {
   ...
   if(ucGetWarnBitWN(ptLWD->tHFD.tHF.ucId, ucEuFIxc) == 1)
   {
      ...
      ucPCompareRESETCond = (u8Pplacard > ucNegHystPMinc) ? (u8Pplacard - ucNegHystPMinc):0;
...
#else
void PrintEuFCfg(void){
  printf("Parametersatz EuF:\n");
  printf("Drop T °C; MDrop; EU Faktor\n");
  printf(" %d; %d; %d \n",scTDropHTVc,ushMDropHTVc,ucEuFactorEuFc);
}
#endif
