...
unsigned char ucGetPMinCS(void)
{
  return(tCDA.tCalTab.ucPMinAbs);
}

unsigned char ucPutPMinCS( unsigned char ucPMin, unsigned char ucMode )
{
  static const unsigned char aucPMinCSc[4] = {(ucPMinDefc), (ucPMinDefUSc), (ucPMinXLc), (u8_PRC_MinPrs_digits)};
  unsigned char ucRet;
...
unsigned char ucGetPSollCS(unsigned char ucIdX )
{
  if(ucIdX < ucSumWEc)
  {
...
signed char scGetTSollCS(unsigned char ucIdX)
{
  if(ucIdX < ucSumWEc)
  {
...
unsigned short ushGetMSollCS(unsigned char ucIdX)
{
  if(ucIdX < ucSumWEc)
  {
...
unsigned char ucPutCalTabVectorCS(unsigned char ucIdX, unsigned char ucP, signed char scT, unsigned short ushM)
{
  unsigned char ucRet;
...
unsigned char ucPutPSollMinCS(unsigned char ucIdX, unsigned char ucP, unsigned char ucMode)
{
  const unsigned char aucPSollMinCSc[3] = {(ucPSollDefc), (ucPSollDefUSc), (ucPMinSollXLc)};
...
unsigned char ucGetPSollMinCS(unsigned char ucIdX)
{
  unsigned char ucRet;

  if(ucIdX < ucSumWEc)
  {
   ucRet = tCDA.tCalTab.aucPSollMin[ucIdX];
  }
  else{
   ucRet = 0;
  }
...
void InitCS(unsigned char ucInit  )
{
  unsigned char i;
  switch( ucInit )
  {
  ...
  case (unsigned short) 1:
   {
      ...
      for( i = 0; i < ucSumWEc; i++ )
      {
        ...
        tCDA.tCalTab.aucPWarm[i] = ucPwarmDefc; // no US definition available up to now
      }
   }
   break;
  case (unsigned short) 2:
   {
      ...
      for( i = 0; i < ucSumWEc; i++ )
      {
        ...
        tCDA.tCalTab.aucPWarm[i] = ucPwarmDefc; // no XtraLoard definition available yet
      }
   }
   break;
  default:
...
unsigned char ucGetPwarmCS(unsigned char ucIdX)
{
  if(ucIdX < ucSumWEc)
  {
...
void PutPwarmCS(unsigned char Pwarm, unsigned char ucIdX)
{
...
void ResetUSCS(void){
   ...
   for(ucLoop = 0; ucLoop < (unsigned char) sizeof(struct CT); ucLoop++)
   {
      tCDA.ucByte[ucLoop] = ucInitValuec;
   }
}
#endif
