...
{
  ControlWATO( cWATO_START_ALARM, ucTimeInSec );
}

void StopWallocTOAlarm(void)
{
  ControlWATO( cWATO_STOP_ALARM, 0 );
}

void ControlWATO( uint8 ucCommand, uint8 ucTimeInSec )
{
  uint16 ushHelp;
  ...
  switch( ucCommand )
  {
   case cWATO_START_ALARM:
      ...
      if( ucAlZOTimeTicks != (uint8) 0 )
      {
        ...
        if( bGetBitFahrzeugzustandFZZ( cRS_VTHRES ) == TRUE )
        {
          ...
          SetRelAlarmAlZOTimeOS( (uint16) ucAlZOTimeTicks, (uint16) 0 );
        }
      }
      break;
   ...
   case cWATO_RESTART_ALARM:
      ...
      if( ucAlZOTimeTicks != (uint8) 0 )
      {
        ...
        CancelAlarmAlZOTimeOS();
        SetRelAlarmAlZOTimeOS( ucDefWATOTimeInSec, (uint16) 0 );
      }
      break;

   case cWATO_READ_ALARM:

      if( GetAlarmAlZOTimeOS( &ushHelp ) == TRUE )
      {
