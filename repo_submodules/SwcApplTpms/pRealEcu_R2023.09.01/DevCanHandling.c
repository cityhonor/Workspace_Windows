...
static tsCAN_Message DCH_NvMReadWritteResponse;  //!< message to be transmitted on CAN after a successful NvM read/write command was received
...
static void DCH_ActivateDebugCfg(const tsCAN_Message* sDebugCanMsgCfg);

//! Check if NvM access is enabled and if it is so, read data from NvM and also writte into it.
static void DCH_NvMReadWriteDebugHandling(const tsCAN_Message* sMultiplexCanNvMCfg);
...
static void DCH_DebugNvMRead(const tsCAN_Message* CanDebugNvmRead);

//! Write specific bytes from a block in NvM.
static void DCH_DebugNvMWrite(const tsCAN_Message* CanDebugNvmWrite);

//! Update cyclic multiplex messages configuration.
static void DCH_MultiplexMsgUpdate(const tsCAN_Message* sMultiplexCanMsgCfg);
 ...
 *
 * \param[in]   const tsCAN_Message* sMultiplexCanMsgCfg - CAN message received from BCM
 * \return      -
 *
 * \note        -
 * \warning     -
 ...
 *
 * \param[in]   const tsCAN_Message* sMultiplexCanNvMCfg - CAN message received from BCM
 * \return      -
 *
 * \note        -
 * \warning     -
 ...
 *
 * \param[in]   const tsCAN_Message* CanDebugNvmRead - NvM Read information received on CAN
 * \return      -
 *
 * \note        -
 * \warning     -
 ...
 *
 * \param[in]   const tsCAN_Message* CanDebugNvmWrite - CAN debug message from BCM to writte a block
 * \return      -
 *
 * \note        -
 * \warning     -
...
void DCH_CheckCanDebugRequest(const tsCAN_Message* CanDebugRequestMsg)
{
...
void DCH_GetCanDebugResponseData(tsCAN_Message* CanDebugResponseMsg)
{
