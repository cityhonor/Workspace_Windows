...
static void CalcAusbeuteSTATISTICS( uint8 ucSlot );
static void PUTbWUFDActive(boolean value);
static void PUTbActivateWUFDTimerRun(boolean value);
static void PUTbProceedWUFDTimerRun(boolean value);
static void PUTbDeactivateWUFDTimerRun(boolean value);
static boolean GETbExceedVThres(void);
static void PUTbExceedVThres(boolean value);
static void PUTbStatisticActive (uint8 ZomPos, boolean value);
static void STT__InitWUFDBits(void);
...
boolean GETbWUFDActive(void)
{
  return (boolean)(WUFDState.bWUFDActive);
}

boolean GETbActivateWUFDTimerRun(void)
{
  return (boolean)(WUFDState.bActivateWUFDTimerRun);
}

boolean GETbProceedWUFDTimerRun(void)
{
  return (boolean)(WUFDState.bProceedWUFDTimerRun);
}

boolean GETbDeactivateWUFDTimerRun(void)
{
...
void InitSTATISTICS(void)
{
  ...
  {
    (void) ResetCounterInSlotSTATISTICS( ucLoop );
  }
...
uint8 ResetCounterInSlotSTATISTICS( uint8 ucSlot )
{
  ...
  if( ucSlot < cAnzRad )
  {
   ...
   ucAusbeuteSTATISTICS[ucSlot]        = 0;
    ...
    SetRecCounterInEE( 0, ucSlot );
    SetMissCounterInEE( 0, ucSlot );
    SetFailCounterInEE( 0, ucSlot );
  }
  else{
   ucRet = cRetError;
  }
...
{
  return ucAusbeuteSTATISTICS[ucSlot];
}
...
void ReloadSTATISTICS(void)
{
  ...
  for( ucLoop = 0; ucLoop < cAnzRad; ucLoop++ )
  {
   ushGutEmpfCntSTATISTICS[ucLoop] = GetRecCounterInEE( ucLoop );
   ushMissedCntSTATISTICS[ucLoop]  = GetMissCounterInEE( ucLoop );

   usFolgeAusfallCntSTATISTICS[ucLoop] = GetFailCounterInEE( ucLoop );

   if((usFolgeAusfallCntSTATISTICS[ucLoop] < GETusMaxFolgeAusfallEE()) &&
       (DTC_GetActiveStatusOfDTC(ucWUFailERRc[ucLoop]) == 0))
   {
      if((usFolgeAusfallCntSTATISTICS[ucLoop] <= ucMaxFF4RFInterference) &&
          (DTC_GetActiveStatusOfDTC(cZO_ERR_RF_INTERFERENCE)== 0))
      {
        usFolgeAusfallCntSTATISTICS[ucLoop] = 0;
      }
   }

   CalcAusbeuteSTATISTICS( ucLoop );
  }
  STT__InitWUFDBits();
}
...
void StoreSTATISTICS(void)
{
  ...
  for( ucLoop = 0; ucLoop < cAnzRad; ucLoop++ )
  {
    SetRecCounterInEE( ushGutEmpfCntSTATISTICS[ucLoop], ucLoop );
    SetMissCounterInEE( ushMissedCntSTATISTICS[ucLoop], ucLoop );
    SetFailCounterInEE( usFolgeAusfallCntSTATISTICS[ucLoop], ucLoop );
  }
}
...
#else
   ucAusbeuteSTATISTICS[ucSlot] = (uint8) (((uint32) ushGutEmpfCntSTATISTICS[ucSlot] * (uint32) 100) / ulSum);
#endif
  }
  else{
...
uint8 CountValidDatagramSTATISTICS( uint8 ucSlot )
{
  ...
  if( ucSlot < cAnzRad )
  {
   ...
   PUTusFolgeAusfallCntSTATISTICS( 0, ucSlot );
   PUTbStatisticActive(ucSlot, TRUE);

   CalcAusbeuteSTATISTICS( ucSlot );
  }
  else{
   ucRet = cRetError;
  }
...
void ExceedVThresSTATISTICS(void)
{
  ...
  if(GETbWUFDActive() == FALSE)
  {
   ...
   if(GETbActivateWUFDTimerRun() == FALSE)
   {
      ...
      SetRelAlarmAlActivateWUFDOS(cAccMeasInt1a,0);

      PUTbActivateWUFDTimerRun(TRUE);
   }
  }
  ...
  if(GETbProceedWUFDTimerRun() == FALSE)
  {
    ...
    SetRelAlarmAlProceedWUFDOS(cAccMeasInt1b,0);

   PUTbProceedWUFDTimerRun(TRUE);
  }
}
...
void UnderrunVThresSTATISTICS(void)
{
  ...
  PUTbActivateWUFDTimerRun(FALSE);

  CancelAlarmAlProceedWUFDOS();

  PUTbProceedWUFDTimerRun(FALSE);
  ...
  if(GETbWUFDActive() == TRUE)
  {
   if(CarIsDriving() == TRUE)
   {
      ...
      if(GETbDeactivateWUFDTimerRun() == FALSE)
      {
        ...
        SetRelAlarmAlDeactivateWUFDOS(5*cAccMeasInt1b,0);

        PUTbDeactivateWUFDTimerRun(TRUE);
      }
      else{
      }
   }
...
void DeactivateWUFDSTATISTICS(void)
{
  ...
  if(GETbWUFDActive() == TRUE)
  {
   ...
   PUTbWUFDActive(FALSE);

   PUTbDeactivateWUFDTimerRun(FALSE);
  }
  else{
...
void TransmitIntervalCompleteSTATISTICS(void)
{
  ...
  for( ucLoop = 0; ucLoop < cAnzRad; ucLoop++ )
  {
   ...
   if( (ucMarkReceivedDatagram & (uint8) (1 << ucLoop)) != 0x00 )
   {

      PUTusFolgeAusfallCntSTATISTICS( 0, ucLoop );

   }else{

      (void) CountInvalidDatagramSTATISTICS( ucLoop );

      if( GETbWUFDActive() == TRUE)
      {
        PUTbStatisticActive(ucLoop, TRUE);

        if( (RfInterferenceActive() == TRUE) || (GetWUHighTempRD(ucLoop) > 2) )
        {
          if(GETusFolgeAusfallCntSTATISTICS(ucLoop) < 4){
            PUTusFolgeAusfallCntSTATISTICS( GETusFolgeAusfallCntSTATISTICS( ucLoop ) + 1, ucLoop );
          }
        }
        else{
          if(GETusFolgeAusfallCntSTATISTICS(ucLoop) < 250)
          {
            PUTusFolgeAusfallCntSTATISTICS( GETusFolgeAusfallCntSTATISTICS( ucLoop ) + 1, ucLoop );
            SetFailCounterInEE( GETusFolgeAusfallCntSTATISTICS( ucLoop ), ucLoop );
          }
        }
      }
   }

   if(CheckWUFDErr(GETusFolgeAusfallCntSTATISTICS( ucLoop )) == TRUE)
   {
      ClearReDataInSlotPD(ucLoop);
...
void MissedCntStatistics(void)
{
  ...
  if( FALSE != bGetBitBetriebszustandBZ( cER_FINISH | cEIGENRAD ) )
  {
   for( ucLoop = 0; ucLoop < cAnzRad; ucLoop++ )
   {
      ...
      if( (ucMarkReceivedDatagram & (uint8) (1u << ucLoop)) != 0x00 )
      {
        ...
        usLocalCntValue = GETusFolgeAusfallCntSTATISTICS(ucLoop);
        ...
        if( GETbWUFDActive() == TRUE)
        {
          ...
          if( (RfInterferenceActive() == TRUE) || (GetWUHighTempRD(ucLoop) > 2) )
          {
            ...
            if(GETusFolgeAusfallCntSTATISTICS(ucLoop) < ucMaxFF4RFInterference)
            {

              PUTusFolgeAusfallCntSTATISTICS( GETusFolgeAusfallCntSTATISTICS( ucLoop ) + 1u, ucLoop );
            }
          }
          else{

            if(GETusFolgeAusfallCntSTATISTICS(ucLoop) < 8000u)
            {
              PUTusFolgeAusfallCntSTATISTICS( GETusFolgeAusfallCntSTATISTICS( ucLoop ) + 1u, ucLoop );

              //SetFailCounterInEE( GETusFolgeAusfallCntSTATISTICS( ucLoop ), ucLoop );  //not neaded because statistic is saved at IGN off event
            }
          }
        }
      }

      if(CheckWUFDErr(GETusFolgeAusfallCntSTATISTICS( ucLoop ), ucLoop) == TRUE)
      {
        ClearReDataInSlotPD(ucLoop);
        (void)ResetWarnVectorUSWIF(ucLoop);
        (void)ClearWarnOfIdUSWIF( ucLoop );
   ...
   if(bPossibleRfInterference == FALSE) //check if all continuous missed RF counter are below RF-Interference value
   {
   ...
   if( CarIsDriving() == FALSE )
   {
    ...
    // don't know our WS, don't do statistics on them
  }
  ucMarkReceivedDatagram = 0;
}
...
boolean GETbStatisticActiveSTATISTICS( const uint8 col )
{
  boolean bRet = FALSE;
...
uint8 DCM_InvIf_RecepMissedGetCntValue(uint8 u8_HistoryIDValue)
{
  //return (uint8) ( 0x00FFu & GETushMissedCntSTATISTICS(u8_HistoryIDValue) );
  return (uint8)(0x00FFu & (GETusFolgeAusfallCntSTATISTICS(u8_HistoryIDValue) >> 2));
}

void PutMissingErrCnt(uint8 ucCurrentValue)
{
  ucMissingErrCnt = ucCurrentValue;
}

uint8 ucGetMissingErrorCnt(void){
  return ucMissingErrCnt;
}

boolean bGetPossibleRfInterf(void){
  uint8 ucLoop = 0;
  boolean bTmpReturn = FALSE;

  for(ucLoop = 0; ucLoop < cAnzRad; ucLoop++)
  {
   if(GETusFolgeAusfallCntSTATISTICS(ucLoop) >= ucMaxFF4RFInterference) //some sensor is already missing for 2 minutes
   {
      bTmpReturn = TRUE;
      break;
   }
