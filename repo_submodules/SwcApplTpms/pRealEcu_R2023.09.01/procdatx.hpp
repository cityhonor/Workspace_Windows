...
#define cRX_TIME_STAMP_INVALID 0xFFFFFFFFU  //Invalid Time Stamp value

extern uint8 ucRotatSTelIndexGet;

#ifdef BUILD_WITH_UNUSED_FUNCTION
extern uint8 GETucStatusPD( uint8 );
extern void PUTucStatusPD( uint8, uint8 );
#endif
...
#ifdef BUILD_WITH_UNUSED_FUNCTION
extern uint8 GETucRollSwitchPD( uint8 ucZomSlot);
extern void PUTucRollSwitchPD( uint8 ucInData, uint8 ucZomSlot );
extern uint8 GETucRSSIsumPD(uint8 ucZomSlot);
extern uint8 GETucTelTypePD(uint8 ucZomSlot);
#endif

extern uint8 GETucRadpositionPD(uint8 ucZomSlot);

#ifdef G_USE_ROTATION_DIRECTION
extern void PUTucWheelDirPD(uint8 ucInData, uint8 ucZomSlot);
extern uint8 GETucWheelDirPD(uint8 ucZomSlot);
#endif
...
#ifdef G_USE_ROTATION_DIRECTION
extern uint8 ucGetReDataSpinDirPD(void);
#endif //G_USE_ROTATION_DIRECTION

#ifdef IBTCM_SW_ANPASSUNGEN

#else
extern uint8 ucGetReDataRssiValuePD( uint8 ucIx );
#endif

extern uint8 ucGetReDataRssiAvgPD(void);
extern uint16 ushGetReDataTimeStampPD(void);
extern uint8 ucGetReDataTelTypePD(void);                // Telegram type
...
#ifdef BUILD_WITH_UNUSED_FUNCTION
extern uint8  ucGetReDataPreProcRollSwitchPD(void);
extern void   PutReDataPreProcRollSwitchPD( uint8 ucRollSwitch);
#endif
...
#ifdef BUILD_WITH_UNUSED_FUNCTION
extern boolean bIsReMessFailPD( uint8 ZomSlot );
extern boolean bIsReLfTriggeredPD(void);
#endif

#ifdef BUILD_WITH_UNUSED_FUNCTION
extern void DecrAKdefMaxFastModeTimePD(void);
#endif
