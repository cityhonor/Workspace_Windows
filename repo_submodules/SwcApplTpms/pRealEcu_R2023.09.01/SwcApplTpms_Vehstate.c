...
static uint8 ucStopEventsCounter;
       ...
       void    VehStateInit(void);
       static sint8   VehStateGetHysteresisHandle(uint8 signalHandle, uint8* pHystHandle);
void    VehStateTriggerHysteresis(uint8 signalHandle, uint16 actVelocity);
       void    VehStateGetKL15(boolean* bpKL15);
       boolean VehStateGetVehicleRollingState(void);
       boolean VehStateGetRoadmode(void);
       boolean VehStateGetReDiagActiveState(void);
static void    VehStateCallbackVehicleRollingChange(void);
static void    VehStateCallbackReDiagActiveChange(void);
uint8   ReceiveGetVehicleSpeed(uint16* pVehSpeed);
uint8   ReceiveGetDirection(uint8 *pucDirection);
uint8   ReceiveGetOutdoorTemp(sint16* pODTemp);
uint8   ReceiveGetTrefTemperature(sint8* pODTemp);
     boolean ReceiveGetVehicleSpeedValidity(void);
     uint8   ReceiveGetFmmotmax(uint8* pucFmmotmaxPressure);
     void    OperateAtmosphericPressure(void);
...
void VehStateTriggerHysteresis(uint8 signalHandle, uint16 actVelocity)
{
  ...
  {
   { (uint16) ( 7), (uint16) ( 3), VEHSTATE_VEHICLE_SPEED_HYST_ID}
   ,  { (uint16) (30), (uint16) (25), VEHSTATE_REDIAG_ACTIVE_HYST_ID}
   ,  { (uint16)  (0), (uint16)  (0),              VEHSTATE_RESERVED}
  };
#else
  static const tHysteresisData HysteresisData[HYSTERESIS_MAX] =
  {
   { (uint16) ( 7), (uint16) ( 3), &VehStateCallbackVehicleRollingChange}
   ,  { (uint16) (30), (uint16) (25), &VehStateCallbackReDiagActiveChange  }
  };
#endif

  if( VehStateGetHysteresisHandle(signalHandle, &hystHandle) == VEHSTATE_RET_OK )
  {
   ...
   if((HysteresisValues[hystHandle].hystActualValue >= HysteresisData[hystHandle].hystActiveValue)&&
        (HysteresisValues[hystHandle].hystLastValue   <  HysteresisData[hystHandle].hystActiveValue)&&
        (HYSTIsFlagSet(hystHandle) == FALSE                                                        )  )
   {

      HYSTSetFlag(hystHandle);
      ...
      if(HysteresisData[hystHandle].ucCallbackNr == VEHSTATE_REDIAG_ACTIVE_HYST_ID)
      {
      ...
      if( HysteresisData[hystHandle].pCallbackPtr != NULL_PTR )
      {

        (HysteresisData[hystHandle].pCallbackPtr)();
      }
#endif
   }
   else if((HysteresisValues[hystHandle].hystActualValue <= HysteresisData[hystHandle].hystInactiveValue)&&
             (HysteresisValues[hystHandle].hystLastValue   >  HysteresisData[hystHandle].hystInactiveValue)&&
             (HYSTIsFlagSet(hystHandle) == TRUE                                                           )  )
   {

      HYSTClearFlag(hystHandle);
...
boolean VehStateGetVehicleRollingState(void){
  ...
  if(HYSTIsFlagSet(VEHSTATE_VEHICLE_SPEED_HYST_ID))
  {
   bHelp = TRUE;
  }
  else{
...
boolean VehStateGetReDiagActiveState(void){
  ...
  if(HYSTIsFlagSet(VEHSTATE_REDIAG_ACTIVE_HYST_ID))
  {
   bHelp = TRUE;
  }
  else{
...
uint8 ReceiveGetTrefTemperature(sint8* pODTemp)
{
  ...
  l_ucRetVal = ReceiveGetOutdoorTemp(&l_siODTemp);
...
boolean ReceiveGetVehicleSpeedValidity(void){
  ...
  bValid = ReceiveGetVehicleSpeed(&u16Temp);
...
uint8 ReceiveGetFmmotmax(uint8* pucFmmotmaxPressure)
{
  ...
  {
   if( (g_sEnv_Data.ucAthmosPressure >= cFMMOTMAXmin) &&
            (g_sEnv_Data.ucAthmosPressure <= cFMMOTMAXmax)    )
   {
...
uint16 DCM_InvIf_SpeedVehicGetValue(void){
  uint16 LocalVehicleSpeedValue;
  uint8 SpeedValidity;

  SpeedValidity = ReceiveGetVehicleSpeed(&LocalVehicleSpeedValue);

  if(FALSE == SpeedValidity)
  { //error ocoured set speed to 0
...
uint8 DCM_InvIf_PressAmbientGetValue(void){
  return g_sEnv_Data.ucAthmosPressure;
}

uint8 DCM_InvIf_NoiseLevelGetValue(void){
  return (uint8)(g_sEnv_Data.uiNoiseRSSI & 0x00FF);
}

uint8 DCM_InvIf_StopEventsGetCntValue(void){
  return ucStopEventsCounter;
}

uint8 DCM_InvIf_VehDirectionGetValue(void){
