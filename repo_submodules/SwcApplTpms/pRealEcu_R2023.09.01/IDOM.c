...
#else
#define ushMaxOLCtc 1200
#define ushMaxULCtc 1200
#endif
  ...
  case ucRFintc:
   if(tInput.ushCurBaseLevel > (ushOffset + GetParaOverlaps()))
   {
      ...
      if( tInput.ushMaxFFCtVal >= ucMaxFF4RFInterference )
      {
        ...
        StartIDOMAlarm(tInput.ushMaxFFCtVal);
      }
   }
   else{
...
unsigned char GETbStoersender(void){
  return((unsigned char)(ucRFintc == (ucIDOMState & ucRFintc)));
}
#endif
