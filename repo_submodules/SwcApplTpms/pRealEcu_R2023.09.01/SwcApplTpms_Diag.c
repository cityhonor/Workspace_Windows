...
// static uint8 DIAG__U8_ResetReasonOfBCM = 0;
...
static uint8 DIAG__ReqCallback_WriteVehicleSpecificParameters(tsTPMSDiag_Data *PS_MsgContext)
{
  ...
  if( (U8_VarCodPlacardPress >= 60u)    && (U8_VarCodPlacardPress <= 214u) )
  {
      if(U8_VarCodLegislation == E_PRC_LEG)
      {
          ...
          U8_VarCodMinPressThres = u8_PRC_MinPrs_digits; // 1500 mbar (phys) -> the default value for PRC legislation
          U8_VarCodOffroadPlacardPressMin = u8_PRC_Offroad_Press_digits;
      }
      else if(U8_VarCodLegislation == E_EU_KO_LEG)
      {

          U8_VarCodMinPressThres = u8_EU_MinPrs_digits; // 1500 mbar (phys) -> the lower limit value for EU/Korean legislation
          U8_VarCodOffroadPlacardPressMin = u8_EU_Offroad_Press_digits;
      }
      else if(U8_VarCodLegislation == E_US_LEG_NON_EXTRA_LOAD_TIRES)
      {

          U8_VarCodMinPressThres = u8_US_NonExtra_MinPrs_digits; // 1400 mbar (phys) -> the default value for US legislation (non extra load tires)
      ...
      else if(U8_VarCodLegislation == E_US_LEG_EXTRA_LOAD_TIRES)
      {
          ...
          U8_VarCodMinPressThres = u8_US_Extra_MinPrs_digits; // 1600 mbar (phys) -> the default value for US legislation (extra load tires)
...
static uint8 DIAG__ReqCallback_WriteAdditionalPressureOfMinimumPressure(tsTPMSDiag_Data *PS_MsgContext)
{
  ...
  if(GETucVarCodLegislationEE() == E_EU_KO_LEG)
  {
          ...
          U8_LowLimitOfPMIN = u8_EU_MinPrs_digits; // 1500 mbar (phys) -> the lower limit value for EU/Korean legislation
  }
  else if(GETucVarCodLegislationEE() == E_US_LEG_NON_EXTRA_LOAD_TIRES)
  {

          U8_LowLimitOfPMIN = u8_US_NonExtra_MinPrs_digits; // 1400 mbar (phys) -> the lower limit value for US legislation (non extra load tires)
  }
  else if(GETucVarCodLegislationEE() == E_US_LEG_EXTRA_LOAD_TIRES)
  {

          U8_LowLimitOfPMIN = u8_US_Extra_MinPrs_digits; // 1600 mbar (phys) -> the lower limit value for US legislation (extra load tires)
  }
  else // (GETucVarCodLegislationEE() == E_PRC_LEG)
  {

          U8_LowLimitOfPMIN = u8_PRC_MinPrs_digits; // 1500 mbar (phys) -> the default value for PRC legislation
  }
...
static uint8 DIAG__ReqCallback_ReadBCMPosition(tsTPMSDiag_Data *PS_MsgContext)
{
  ...
  { // positive response, don't change value
  }
  else{
...
/*!************************************************************************************************
 * \brief This function writes a new BCM position.
 *
 * \param[in]     PS_MsgContext->pucReqData:
 *                - Points to the first service request data byte.
 * \param[out]    PS_MsgContext->pucResData:
 *                - Points to the first writable byte for the service response data.
 * \param[in]     PS_MsgContext->uiReqDataLen:
 *                - Contains the count of the service request data bytes (Sid is excluded).
 * \param[out]    PS_MsgContext->puiResDataLen:
 *                - Must be initialized with the count of the service response data bytes (Sid is excluded).
 * \return      0x00 if positive response or a specific negative response code
 *
 * \note        -
 * \warning     -
 *************************************************************************************************/
...
/*!************************************************************************************************
 * \brief Write new ABS ticks value on the front and on the rear axle.
 *
 * \param[in]     PS_MsgContext->pucReqData:
 *                - Points to the first service request data byte.
 * \param[out]    PS_MsgContext->pucResData:
...
static uint8 DIAG__ReqCallback_WriteABSTicksValue(tsTPMSDiag_Data *PS_MsgContext)
{
  ...
  LocalABSTicksInOneRev.ucFrontAxisABSTicks = 0; /* because of MISRA, needs to be initialized here */
  LocalABSTicksInOneRev.ucRearAxisABSTicks = 0; /* because of MISRA, needs to be initialized here */
  ...
  { // valid values
   ...
   { /* do nothing, same values are already in NvM */
   }
   else{ /* different values than the ones from NvM, overwrite them */
...
/*!************************************************************************************************
 * \brief Read the RSSI Noise Level Threshold written in the memory.
 *
 * \param[in]     PS_MsgContext->pucReqData:
 *                - Points to the first service request data byte.
 * \param[out]    PS_MsgContext->pucResData:
 ...
 * \param[out]    PS_MsgContext->puiResDataLen:
 *                - Must be initialized with the count of the service response data bytes (Sid is excluded).
 * \return      0x00 for positive response(no negative response available for read)
 *
 * \note        -
 * \warning     -
 *************************************************************************************************/
...
/*!************************************************************************************************
 * \brief Write a RSSI Noise Level Threshold for setting the DTC for RF interference.
 *
 * \param[in]     PS_MsgContext->pucReqData:
 *                - Points to the first service request data byte.
 * \param[out]    PS_MsgContext->pucResData:
...
{
  uint8 U8_RetVal = DIAG__CU8_POSITIVE_RESPONSE;
  U16_RespLenght = 0;  //U16_RespLenght = 1;
  AU8_ResData[0] =0;
...
static void DIAG__GetWSDataForPosition(const uint8 CU8_PosIndex, uint8* PU8_InterfaceData)
{
  ...
  PU8_InterfaceData[U8_Idx] = (0xFFu & ((U8_LocalReifendruckPD + 2) / 4));
  //PU8_InterfaceData[U8_Idx] = (uint8)( 0xFFu & GETucVarCodPlacardPressEE() );
  U8_Idx += 1;
...
static uint8 DIAG__PutNewIDAndResetWarning(uint8* PU8_ReqData)
{
  ...
  U8_Ret = ucSetID(&U32_REId, PU8_ReqData);//, (uint8) 1);
...
uint8 HufIf_DiagReqCallback(const enum_TPMSDiagnosticRequest eDiagRequest, tsTPMSDiag_Data *PS_MsgContext)
{
  uint8 U8_RetVal = DIAG__CU8_POSITIVE_RESPONSE;
  uint8 U8_Index = 0;

  for(U8_Index = 0; U8_Index < E_TPMS_DIAG_MAX_NUMBER; U8_Index++)
  {
   if(eDiagRequest == DIAG__AS_ReqHandlerArray[U8_Index].eDiagRequest)
   {
      if(((void *) 0) != DIAG__AS_ReqHandlerArray[U8_Index].PF_ReqCallback)
      {
        U8_RetVal = DIAG__AS_ReqHandlerArray[U8_Index].PF_ReqCallback(PS_MsgContext);
      }
      else{
  ...
  if(U8_Index >= E_TPMS_DIAG_MAX_NUMBER )
  {
  ...
  ClientIf_Debug_DiagReqCallback(eDiagRequest,PS_MsgContext,U8_RetVal);
